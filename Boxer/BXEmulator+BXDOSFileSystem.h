...
/// The \c BXDOSFileSystem category extends \c BXEmulator to expose methods for inspecting and controlling
...
/// all drive letters in order, including reserved letters
@property (class, readonly, copy) NSArray<NSString*> *driveLetters;
/// letters appropriate for floppy drives (A-W)
@property (class, readonly, copy) NSArray<NSString*> *floppyDriveLetters;
/// letters appropriate for hard disk drives (C-W)
@property (class, readonly, copy) NSArray<NSString*> *hardDriveLetters;
/// letters appropriate for CD-ROM drives (D-W)
@property (class, readonly, copy) NSArray<NSString*> *CDROMDriveLetters;


/// Generates a drive from the specified mount/imgmount command string,
/// with or without the mount/imgmount command name.
/// If \c baseURL is specified, relative paths in the command will be resolved
/// relative to that.
/// Returns \c nil and populates \c outError if the mount command could not be parsed.
...
/// Unmount the specified drive letter. Returns \c YES if drive was successfully unmounted,
/// or \c NO and populates outError otherwise.
...
/// Returns \c YES if successful, \c NO and populates outError on failure.
...
/// or \c nil if no drive contains the specified URL. This is slower than \c URLIsAccessibleInDOS:
...
/// or \c nil if that resource is not currently accessible from DOS.
...
/// Returns \c nil if the file does not exist or is not accessible via the OS X filesystem.
...
/// or \c nil if the path is not currently accessible from DOS.
