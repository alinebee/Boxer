...
- (void) setGameboxName: (NSString *)newName
{
	if ([newName length] && ![newName isEqualToString: [self gameboxName]])
	{
		...
		if (moved)
		{
			...
			if (hasAutoGeneratedIcon)
			{
				BXGameEra era = [self sourcePath] ? [BXGameProfile eraOfGameAtPath: [self sourcePath]] : BXUnknownEra;
				NSImage *icon = [[self class] bootlegCoverArtForGamePackage: movedPackage withEra: era];
				[movedPackage setCoverArt: icon];
			}
		}
	}
}
...
{	
	...
	if (gamebox)
	{
		...
		//If no suitable icon was found, generate a bootleg icon based on the name of the gamebox
		if (!icon)
		{
			BXGameEra era = [BXGameProfile eraOfGameAtPath: [self sourcePath]];
			icon = [[self class] bootlegCoverArtForGamePackage: gamebox withEra: era];
			//Flag this as autogenerated so we know to regenerate it if the user renames the gamebox
			hasAutoGeneratedIcon = YES;
		}
		[gamebox setCoverArt: icon];
