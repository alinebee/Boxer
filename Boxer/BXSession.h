...
/// Used by \c openURLInDOS: to decide what to do after the requested program exits or the directory has been changed.
...
/// \c BXSession is an \c NSDocument subclass which encapsulates a single DOS emulation session.
/// It manages an underlying \c BXEmulator (configuring, starting and stopping it), reads and writes
/// the gamebox for the session (if any), and creates the various window controllers for the session.
///
/// \c BXSession is extended by separate categories that encapsulate different aspects of functionality
/// (mostly for controlling the emulator.)
...
/// Will be \c nil if an executable file or folder was opened outside of a gamebox.
...
/// Will only be \c YES while the session is idling at the DOS prompt.
...
/// Returns \c NO if this is a standalone game bundle with only a single launcher
...
/// Whether this session is a game import. Returns \c NO by default; overridden by BXImportSession to return YES.
