...
/* Added 2009-01-27 by Alun Bestor to allow AudioFilePlayer_OpenFile to swap endianness of AIFF metadata when appropriate */
...
static int    AudioFilePlayer_OpenFile (AudioFilePlayer *afp, const FSRef *inRef, SInt64 *outFileDataSize)
{
	...
	/* Added 2009-01-27 by Alun Bestor to translate AIFF metadata from big-endian to the appropriate endianness for the current architecture */
	chunkHeader.ckID		= SDL_SwapBE32(chunkHeader.ckID);
	chunkHeader.formType	= SDL_SwapBE32(chunkHeader.formType);
    ...
    */
	...
	ssndData.blockSize	= SDL_SwapBE32(ssndData.blockSize);
