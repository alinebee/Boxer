	  ...
	  case mouseDown: {
		...
		switch (area) {
		  ...
		  case inContent:
			...
			/* Edited 2009-01-18 by Alun Bestor: changed to ctrl-click acting as RMB to be consistent with OSX conventions,
			also supports both left and right modifier keys */
			if ( event.modifiers & optionKey || event.modifiers & rightOptionKey ) {
				mouse_button = 2;
			} else if ( event.modifiers & controlKey || event.modifiers & rightControlKey /* was cmdKey */ ) {
				mouse_button = 3;
			} else {
				mouse_button = 3;
			}
